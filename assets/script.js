...
function generatePassword() {
  ...
  var userWantsLowercase = window.confirm(
    "would you lie to include lowercase password in your password?"
  );
  var userWantsUppercase = window.confirm(
    "would you to include Uppercase letters in your password?"
  );
  var userWantsNumber = window.confirm(
    "would you like to include number in your line?"
  );
  ...
  for (var i = 0; i < passwordLength; i++) {
    randomChar = Math.floor(Math.random() * optionCart.length);

    generatedPassword += optionCart[randomChar];
  }
